import pytest
from cogs.ministral_cog import MinistralCog
...
@pytest.fixture
def cog(mock_bot):
    return MinistralCog(mock_bot)

def test_cog_initialization(cog):
    assert cog.name == "Ministral"
    assert cog.nickname == "Ministral"
    assert cog.model == "mistralai/ministral-8b"
