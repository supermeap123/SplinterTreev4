import pytest
from cogs.dolphin_cog import DolphinCog
...
@pytest.fixture
def cog(mock_bot):
    return DolphinCog(mock_bot)

def test_cog_initialization(cog):
    assert cog.name == "Dolphin"
    assert cog.nickname == "Dolphin"
    assert cog.model == "cognitivecomputations/dolphin-mixtral-8x22b"
