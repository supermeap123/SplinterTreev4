...
class MythomaxCog(BaseCog):
    def __init__(self, bot):
        ...
        try:
            ...
            logging.error(f"[Mythomax] Failed to load temperatures.json: {e}")
            self.temperatures = {}
    ...
    async def generate_response(self, message):
        ...
        try:
            ...
            logging.debug(f"[Mythomax] Sending {len(messages)} messages to API")
            logging.debug(f"[Mythomax] Formatted prompt: {formatted_prompt}")

            # Get temperature for this agent
            temperature = self.get_temperature()
            logging.debug(f"[Mythomax] Using temperature: {temperature}")
            ...
            logging.error(f"Error processing message for Mythomax: {e}")
            return None
...
async def setup(bot):
    ...
    try:
        ...
        logging.error(f"[Mythomax] Failed to register cog: {e}", exc_info=True)
        raise