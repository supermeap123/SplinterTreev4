...
class O1MiniCog(BaseCog):
    def __init__(self, bot):
        ...
        try:
            ...
            logging.error(f"[O1-Mini] Failed to load temperatures.json: {e}")
            self.temperatures = {}
    ...
    async def generate_response(self, message):
        ...
        try:
            ...
            logging.error(f"Error processing message for O1-Mini: {e}")
            return None
...
async def setup(bot):
    ...
    try:
        ...
        logging.error(f"[O1-Mini] Failed to register cog: {e}", exc_info=True)
        raise
