...
class MagnumCog(BaseCog):
    ...
    async def on_message(self, message):
        ...
        if is_triggered:
            logging.debug(f"[Magnum] Triggered by message: {message.content}")
            async with message.channel.typing():
                try:
                    # Process message and get response
                    logging.debug(f"[Magnum] Processing message with provider: {self.provider}, model: {self.model}")
                    response = await self.process_message(message)
                    
                    if response:
                        logging.debug(f"[Magnum] Got response: {response[:100]}...")
                        ...
                        try:
                            ...
                            logging.debug(f"[Magnum] Logged interaction for user {message.author.id}")
                        except Exception as e:
                            logging.error(f"[Magnum] Failed to log interaction: {str(e)}", exc_info=True)
                    else:
                        logging.error("[Magnum] No response received from API")
                        await message.add_reaction('❌')
                        await message.reply(f"[{self.name}] Failed to generate a response. Please try again.")

                except Exception as e:
                    logging.error(f"[Magnum] Error in message handling: {str(e)}", exc_info=True)
                    await message.add_reaction('❌')
                    error_msg = str(e)
