...
class MinistralCog(BaseCog):
    ...
    async def handle_message(self, message):
        ...
        if is_triggered:
            logging.debug(f"[Ministral] Triggered by message: {message.content}")
            async with message.channel.typing():
                try:
                    # Process message and get response
                    logging.debug(f"[Ministral] Processing message with provider: {self.provider}, model: {self.model}")
                    response = await self.process_message(message)
                    
                    if response:
                        logging.debug(f"[Ministral] Got response: {response[:100]}...")
                        ...
                        try:
                            ...
                            logging.debug(f"[Ministral] Logged interaction for user {message.author.id}")
                        except Exception as e:
                            logging.error(f"[Ministral] Failed to log interaction: {str(e)}", exc_info=True)
                    else:
                        logging.error("[Ministral] No response received from API")
                        await message.add_reaction('❌')
                        await message.reply(f"[{self.name}] Failed to generate a response. Please try again.")

                except Exception as e:
                    logging.error(f"[Ministral] Error in message handling: {str(e)}", exc_info=True)
                    await message.add_reaction('❌')
                    error_msg = str(e)
