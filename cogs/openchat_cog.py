...
class OpenChatCog(BaseCog):
    ...
    async def on_message(self, message):
        ...
        if is_triggered:
            logging.debug(f"[OpenChat] Triggered by message: {message.content}")
                ...
                try:
                    ...
                    if response:
                        ...
                        try:
                            ...
                            logging.debug(f"[OpenChat] Logged interaction for user {message.author.id}")
                        except Exception as e:
                            logging.error(f"[OpenChat] Failed to log interaction: {str(e)}", exc_info=True)
                    else:
                        logging.error("[OpenChat] No response received from API")
                        await message.add_reaction('❌')
                        await message.reply(f"[{self.name}] Failed to generate a response. Please try again.")

                except Exception as e:
                    logging.error(f"[OpenChat] Error in message handling: {str(e)}", exc_info=True)
                    await message.add_reaction('❌')
                    error_msg = str(e)
